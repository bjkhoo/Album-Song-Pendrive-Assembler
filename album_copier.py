    ...
    try:
        if system == "Windows":
            ...
            if result.returncode == 0:
                ...
                # Fallback: try using the label command
                subprocess.run(
            ...
            if result.returncode == 0:
                ...
                print(f"  ⚠️  Could not rename drive: {result.stderr.strip()}")
...
def main():
    ...
    for song in SONG_LIST:
        if not os.path.isfile(os.path.join(source_dir, song)):
            missing.append(song)

    if missing:
        print(f"  ⚠️  WARNING: {len(missing)} song(s) missing from '{SOURCE_FOLDER}':")
        for m in missing:
            print(f"     - {m}")
        print()
        proceed = input("  Continue anyway? (Y/N): ").strip().upper()
        if proceed != "Y":
            print("  Exiting.")
            sys.exit(0)
        print()
    ...
    while True:
        ...
        for drive in pen_drives:
            ...
            print(f"  📌 Found: {drive} ({label}, {format_size(free_space)} free)")
            print()

            user_input = input(f"  Press Enter to start copying to {drive} (S to skip, Q to quit): ").strip().upper()
            if user_input == "Q":
